 ...
 * 
 */
public class TrendValueESAbmParams extends SimulationParameters {
    
    private static final Logger logger = (Logger)LoggerFactory.getLogger(TrendValueESAbmParams.class.getSimpleName());
    ...
    /*
     * Confidence level of the VaR and ES models
     */
    public double confLevelVar = 0;         // confidence level of the VaR model
    public double confLevelEs  = 0;         // confidence level of the ES model
    ...
    public int volWindowStratTrend      = 0;     // size of the window to compute the volatility for the Trend strategy
    public double probShortSellingTrend = 0;     // percentage of TRENDs (0-1) allowed to short-sell
    public double probVarTrend          = 0;     // percentage of TRENDs (0-1) who use VaR
    public double varLimitTrendMin      = 0;     // VaR limit
    public double varLimitTrendMax      = 0;
    public int volWindowTrendMin        = 0;     // size of the window to compute the volatility for the VaR/ES calculations
    public int volWindowTrendMax        = 0;
    public double probEsTrend           = 0;     // percentage of TRENDs (0-1) who use ES
    public double esLimitTrendMin       = 0;     // ES limit
    public double esLimitTrendMax       = 0;
    ...
    public double probVarFund           = 0;     // percentage of FUNDs (0-1) who use VaR
    public double varLimitFundMin       = 0;     // VaR limit
    public double varLimitFundMax       = 0;
    public int volWindowFundMin         = 0;     // size of the window to compute the volatility for the VaR/ES calculations
    public int volWindowFundMax         = 0;
    public double probEsFund            = 0;     // percentage of FUNDs (0-1) who use ES
    public double esLimitFundMin        = 0;     // ES limit
    public double esLimitFundMax        = 0;
...
//            return -1;
    ...
    public Boolean validate() {
        ...
        Assertion.assertStrict(confLevelVar >= 0 && confLevelVar <= 1 , Assertion.Level.ERR, "confLevelVar needs to be between 0 and 1");
        Assertion.assertStrict(confLevelEs >= 0 && confLevelEs <= 1 , Assertion.Level.ERR, "confLevelEs needs to be between 0 and 1");
        Assertion.assertStrict((numTrends > 0 || numFunds > 0), Assertion.Level.ERR, "Either numTrends = " + numTrends 
        		+ "  or numFunds = " + numFunds + "  need to be '> 0'");
//        Assertion.assertStrict(normPeriod >= 0, Assertion.Level.ERR, "normPeriod = " + normPeriod + ": needs to be '>= 0'");
        ...
        Assertion.assertStrict(volWindowStratTrend >= 0, Assertion.Level.ERR, "volWindowStratTrend needs to be '>= 0'");
        ...
        Assertion.assertStrict(probVarTrend >= 0 && probVarTrend <= 1 , Assertion.Level.ERR, "probVarTrend needs to be between 0 and 1");
        Assertion.assertStrict(probEsTrend >= 0 && probEsTrend <= 1 , Assertion.Level.ERR, "probEsTrend needs to be between 0 and 1");
        Assertion.assertStrict((varLimitTrendMin > 0 && varLimitTrendMax > 0), Assertion.Level.ERR, "varLimitTrend needs to be '> 0'");
        Assertion.assertStrict((esLimitTrendMin > 0 && esLimitTrendMax > 0), Assertion.Level.ERR, "esLimitTrend needs to be '> 0'");
        Assertion.assertStrict(volWindowTrendMin > 0 && volWindowTrendMax > 0, Assertion.Level.ERR, "volWindowTrend needs to be '> 0'");
...
//        Assertion.assertStrict((exitThresholdMin > 0 && exitThresholdMax > 0), Assertion.Level.ERR, "exitThreshold needs to be '> 0'");
        ...
        Assertion.assertStrict(probVarFund >= 0 && probVarFund <= 1 , Assertion.Level.ERR, "probVarFund needs to be between 0 and 1");
        Assertion.assertStrict(probEsFund >= 0 && probEsFund <= 1 , Assertion.Level.ERR, "probEsFund needs to be between 0 and 1");
        Assertion.assertStrict(probVarFund + probEsFund <= 1, Assertion.Level.ERR, "probVarFund + probEsFund needs to be '<= 1'");
        Assertion.assertStrict(probVarTrend + probEsTrend <= 1, Assertion.Level.ERR, "probVarTrend + probEsTrend needs to be '<= 1'");
        Assertion.assertStrict((varLimitFundMin > 0 && varLimitFundMax > 0), Assertion.Level.ERR, "varLimitFund needs to be '> 0'");
        Assertion.assertStrict((esLimitFundMin > 0 && esLimitFundMax > 0), Assertion.Level.ERR, "esLimitFund needs to be '> 0'");
        Assertion.assertStrict(volWindowFundMin > 0 && volWindowFundMax > 0, Assertion.Level.ERR, "volWindowFund needs to be '> 0'");
...
//    }
    ...
    public static void writeParamDefinition(String file) throws FileNotFoundException {
        writeParamsDefinition(file, new TrendValueESAbmParams());
    }
     ...
     * @throws FileNotFoundException 
     */
    public static TrendValueESAbmParams readParameters(String file) throws FileNotFoundException {

        TrendValueESAbmParams params = (TrendValueESAbmParams) readParams(file, new TrendValueESAbmParams());
