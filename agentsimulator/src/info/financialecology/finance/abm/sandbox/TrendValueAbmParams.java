...
public class TrendValueAbmParams extends SimulationParameters {
    
    private static final Logger logger = (Logger)LoggerFactory.getLogger(TrendValueAbmParams.class.getSimpleName());
...
//    public int normPeriod       = 0;      // DEPRECATED - range of input data for automatically normalising the orders of VALUE and TREND traders, in ticks
    ...
    public int volWindow;                   // window to calculate the volatility of each asset
...
//        Assertion.assertStrict(normPeriod >= 0, Assertion.Level.ERR, "normPeriod = " + normPeriod + ": needs to be '>= 0'");
        Assertion.assertStrict(volWindow > 0, Assertion.Level.ERR, "volWindow needs to be '> 0'");
...
//        Assertion.assertStrict((exitThresholdMin > 0 && exitThresholdMax > 0), Assertion.Level.ERR, "exitThreshold needs to be '> 0'");
        ...
        Assertion.assertStrict((validated_p1 == true) && (validated_p2 == true), Level.ERR, "Parameters for the the exogenous value process are not well-defined " +
  		"(some are provided while others are omitted for either the sinus generator or the Brownian process)");
...
//    }
    ...
    public static void writeParamDefinition(String file) throws FileNotFoundException {
        writeParamsDefinition(file, new TrendValueAbmParams());
    }
     ...
     * @throws FileNotFoundException 
     */
    public static TrendValueAbmParams readParameters(String file) throws FileNotFoundException {

        TrendValueAbmParams params = (TrendValueAbmParams) readParams(file, new TrendValueAbmParams());
