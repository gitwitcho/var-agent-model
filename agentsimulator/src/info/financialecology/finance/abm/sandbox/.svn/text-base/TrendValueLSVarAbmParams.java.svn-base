 ...
 * 
 */
public class TrendValueLSVarAbmParams extends SimulationParameters {
    
    private static final Logger logger = (Logger)LoggerFactory.getLogger(TrendValueLSVarAbmParams.class.getSimpleName());
...
//    public int normPeriod       = 0;      // DEPRECATED - range of input data for automatically normalising the orders of VALUE and TREND traders, in ticks
    ...
    public int volWindowVarTrendMin     = 0;     // size of the window to compute the volatility for the VaR calculations
    public int volWindowVarTrendMax     = 0;
    ...
    public int volWindowVarFundMin      = 0;     // size of the window to compute the volatility for the VaR calculations
    public int volWindowVarFundMax      = 0;
    ...
    public int volWindowStratLS            = 0;     // size of the window to compute the volatility for the LS strategy
    ...
    public double probVarLS                = 0;     // percentage of LS's (0-1) who use VaR
    public double varLimitLSMin            = 0;     // VaR limit
    public double varLimitLSMax            = 0;
    public int volWindowVarLSMin           = 0;     // size of the window to compute the volatility for the VaR calculations
    public int volWindowVarLSMax           = 0;
...
//        Assertion.assertStrict(normPeriod >= 0, Assertion.Level.ERR, "normPeriod = " + normPeriod + ": needs to be '>= 0'");
        ...
        Assertion.assertStrict(volWindowVarTrendMin > 0 && volWindowVarTrendMax > 0, Assertion.Level.ERR, "volWindowVarTrend needs to be '> 0'");
...
//        Assertion.assertStrict((exitThresholdMin > 0 && exitThresholdMax > 0), Assertion.Level.ERR, "exitThreshold needs to be '> 0'");
        ...
        Assertion.assertStrict(volWindowVarFundMin > 0 && volWindowVarFundMax > 0, Assertion.Level.ERR, "volWindowVarFund needs to be '> 0'");
        ...
        Assertion.assertStrict(volWindowStratLS >= 0, Assertion.Level.ERR, "volWindowStratLS needs to be '>= 0'");
        ...
        Assertion.assertStrict(probVarLS >= 0 && probVarLS <= 1 , Assertion.Level.ERR, "probVarLS needs to be between 0 and 1");
        Assertion.assertStrict((varLimitLSMin > 0 && varLimitLSMax > 0), Assertion.Level.ERR, "varLimitLS needs to be '> 0'");
        Assertion.assertStrict(volWindowVarLSMin > 0 && volWindowVarLSMax > 0, Assertion.Level.ERR, "volWindowVarLS needs to be '> 0'");
...
//    }
    ...
    public static void writeParamDefinition(String file) throws FileNotFoundException {
        writeParamsDefinition(file, new TrendValueLSVarAbmParams());
    }
     ...
     * @throws FileNotFoundException 
     */
    public static TrendValueLSVarAbmParams readParameters(String file) throws FileNotFoundException {

        TrendValueLSVarAbmParams params = (TrendValueLSVarAbmParams) readParams(file, new TrendValueLSVarAbmParams());
