/*
 * Copyright (c) 2024-2025 Gilbert Peffer, Barbara Llacay
 * 
 ...
 *
 */
public class TrendValueESAbmSimulator extends AbstractSimulator {
    ...
    private static final Logger logger = (Logger)LoggerFactory.getLogger(TrendValueESAbmSimulator.class.getSimpleName());
    
    
    /**
     * Constructor. Creates the scheduler ({@link ISchedule}) and the {@link ShareMarket}.
     */
    public TrendValueESAbmSimulator() {
        
        logger.trace("Calling: TrendValueESAbmSimulator()");
     ...
     */
    public TrendValueESAbmSimulator(String pTrend, String pValue) {
        
        this();

        logger.trace("Calling: TrendValueESAbmSimulator(String, String)");
...
//        HashMap<String, Trader> trendFollowers = getTrendFollowers();
    ...
    public void run() {
        ...
        Assertion.assertStrict((!getTrendFollowers().isEmpty() || !getValueInvestors().isEmpty()), 
        		Level.ERR, "There are no trend followers and value investors in the market '" + market.getId() + "'");
     ...
     */
	public DoubleTimeSeries getFundTotalVarReducedOrders(String secId) { 
    ...
    /**
     *  Calculate the aggregated time series of orders sent to reduce the ES of
     *  fundamental investors' portfolios (sum of reduction orders with their sign)
     *  
     *  @param secId the unique security identifier
     *  @return the time series of aggregated reduction orders
     */
	public DoubleTimeSeries getFundTotalEsReducedOrders(String secId) { 
        ...
        for (String key : valueTraders.keySet()) {
        	DoubleTimeSeries orders = new DoubleTimeSeries();
        	Trader fund = valueTraders.get(key);
        	DoubleTimeSeries fundReducedPos = fund.getPortfolioEsReductions().getTsPosition(secId);
     ...
     *  @return the time series of aggregated reduction orders
     */
	public DoubleTimeSeries getTrendTotalVarReducedOrders(String secId) { 
     ...
     *  @return the time series of aggregated reduction orders
     */
	public DoubleTimeSeries getTrendTotalEsReducedOrders(String secId) { 
        ...
        for (String key : trendTraders.keySet()) {
        	DoubleTimeSeries orders = new DoubleTimeSeries();
        	Trader trend = trendTraders.get(key);
        	DoubleTimeSeries trendReducedPos = trend.getPortfolioEsReductions().getTsPosition(secId);
     ...
     */
	public DoubleTimeSeries getFundVarReducedVolume(String secId) { 
    ...
    /**
     *  Calculate the total volume time series (sum of absolute orders) of trades made by
     *  fundamental investors to reduce the ES of their portfolio  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of reduction volume
     */
	public DoubleTimeSeries getFundEsReducedVolume(String secId) { 
     ...
     *  @return the time series of reduction volume
     */
	public DoubleTimeSeries getTrendVarReducedVolume(String secId) { 
    ...
    /**
     *  Calculate the total volume time series (sum of absolute orders) of trades made by
     *  trend followers to reduce the ES of their portfolio  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of reduction volume
     */
	public DoubleTimeSeries getTrendEsReducedVolume(String secId) { 
    ...
    /**
     *  Calculate the number of FUND agents that hit their VaR limit and are forced
     *  to reduce their positions at each time step.
     *  
     *  @param secId the unique security identifier
     *  @return the time series of the number of fundamental investors being forced to reduce their positions
     */
	public DoubleTimeSeries getNumberFundsHittingVarLimit(String secId) { 
		
		HashMap<String, Trader> valueTraders = getValueInvestors();
		DoubleTimeSeries tsNumberFundsLiquidating = new DoubleTimeSeries();
		
		// No value investors? Set entries in tsNumberFundsLiquidating to zero and return
		if (valueTraders.size() == 0) {
		    tsNumberFundsLiquidating.fillWithConstants((int) nTicks, 0.0);
		    return tsNumberFundsLiquidating;
		}
		
		// Create a time series list of the number of fundamental investors triggering their VaR limit at each time step        
        for (int i = 0; i < nTicks; i++) { 
        	int nFundsLiquidating = 0;
        	
            for (String key : valueTraders.keySet()) {
            	Trader fund = valueTraders.get(key);
            	if (fund.getPortfolioVarReductions().getTsPosition(secId).get(i) != 0)
            		nFundsLiquidating++;
            	
            	tsNumberFundsLiquidating.add(i, nFundsLiquidating);
            }
        }
    ...
    /**
     *  Calculate the number of TREND agents that hit their VaR limit and are forced
     *  to reduce their positions at each time step.
     *  
     *  @param secId the unique security identifier
     *  @return the time series of the number of trend followers being forced to reduce their positions
     */
	public DoubleTimeSeries getNumberTrendsHittingVarLimit(String secId) { 
		
		HashMap<String, Trader> trendTraders = getTrendFollowers();
		DoubleTimeSeries tsNumberTrendsLiquidating = new DoubleTimeSeries();
		
		// No trend followers? Set entries in tsNumberTrendsLiquidating to zero and return
		if (trendTraders.size() == 0) {
		    tsNumberTrendsLiquidating.fillWithConstants((int) nTicks, 0.0);
		    return tsNumberTrendsLiquidating;
		}
		
		// Create a time series list of the number of trend followers triggering their VaR limit at each time step        
        for (int i = 0; i < nTicks; i++) { 
        	int nTrendsLiquidating = 0;
        	
            for (String key : trendTraders.keySet()) {
            	Trader trend = trendTraders.get(key);
            	if (trend.getPortfolioVarReductions().getTsPosition(secId).get(i) != 0)
            		nTrendsLiquidating++;
            	
            	tsNumberTrendsLiquidating.add(i, nTrendsLiquidating);
            }
        }
    ...
    /**
     *  Calculate the number of FUND agents that hit their ES limit and are forced
     *  to reduce their positions at each time step.
     *  
     *  @param secId the unique security identifier
     *  @return the time series of the number of fundamental investors being forced to reduce their positions
     */
	public DoubleTimeSeries getNumberFundsHittingEsLimit(String secId) { 
		...
		// Create a time series list of the number of fundamental investors triggering their ES limit at each time step        
        for (int i = 0; i < nTicks; i++) { 
        	int nFundsLiquidating = 0;
        	
            for (String key : valueTraders.keySet()) {
            	Trader fund = valueTraders.get(key);
            	if (fund.getPortfolioEsReductions().getTsPosition(secId).get(i) != 0)
            		nFundsLiquidating++;
    ...
    /**
     *  Calculate the number of TREND agents that hit their ES limit and are forced
     *  to reduce their positions at each time step.
     *  
     *  @param secId the unique security identifier
     *  @return the time series of the number of trend followers being forced to reduce their positions
     */
	public DoubleTimeSeries getNumberTrendsHittingEsLimit(String secId) { 
		...
		// Create a time series list of the number of trend followers triggering their ES limit at each time step        
        for (int i = 0; i < nTicks; i++) { 
        	int nTrendsLiquidating = 0;
        	
            for (String key : trendTraders.keySet()) {
            	Trader trend = trendTraders.get(key);
            	if (trend.getPortfolioEsReductions().getTsPosition(secId).get(i) != 0)
            		nTrendsLiquidating++;
     ...
     */
	public DoubleTimeSeries getFundTotalVarSelloffOrders(String secId) { 
    ...
    /**
     *  Calculate the aggregated time series of sell-off orders due to ES of
     *  fundamental investors' portfolios (sum of sell-off orders with their sign)
     *  
     *  @param secId the unique security identifier
     *  @return the time series of aggregated sell-off orders
     */
	public DoubleTimeSeries getFundTotalEsSelloffOrders(String secId) { 
        ...
        for (String key : valueTraders.keySet()) {
        	DoubleTimeSeries orders = new DoubleTimeSeries();
        	Trader fund = valueTraders.get(key);
        	DoubleTimeSeries fundSelloffPos = fund.getTsEsSelloff().get(secId);
     ...
     *  @return the time series of aggregated sell-off orders
     */
	public DoubleTimeSeries getTrendTotalVarSelloffOrders(String secId) { 
     ...
     *  @return the time series of aggregated sell-off orders
     */
	public DoubleTimeSeries getTrendTotalEsSelloffOrders(String secId) { 
        ...
        for (String key : trendTraders.keySet()) {
        	DoubleTimeSeries orders = new DoubleTimeSeries();
        	Trader trend = trendTraders.get(key);
        	DoubleTimeSeries trendSelloffPos = trend.getTsEsSelloff().get(secId);
     ...
     *  orders in absolute value) due to VaR of fundamental investors  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of sell-off volume
     */
	public DoubleTimeSeries getFundVarSelloffVolume(String secId) { 
    ...
    /**
     *  Calculate the total sell-off volume time series (sum of sell-off 
     *  orders in absolute value) due to ES of fundamental investors  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of sell-off volume
     */
	public DoubleTimeSeries getFundEsSelloffVolume(String secId) { 
    ...
    /**
     *  Calculate the total sell-off volume time series (sum of sell-off 
     *  orders in absolute value) due to VaR of trend followers  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of sell-off volume
     */
	public DoubleTimeSeries getTrendVarSelloffVolume(String secId) { 
    ...
    /**
     *  Calculate the total sell-off volume time series (sum of sell-off 
     *  orders in absolute value) due to ES of trend followers  
     *  
     *  @param secId the unique security identifier
     *  @return the time series of sell-off volume
     */
	public DoubleTimeSeries getTrendEsSelloffVolume(String secId) { 
...
//
    ...
    /**
     *  Calculate the ES time series averaged over fundamental investors  
     *    
     *  @return the time series of ES level
     */
	public DoubleTimeSeries getFundAvgEs() { 
		
		HashMap<String, Trader> valueTraders = getValueInvestors();
		DoubleTimeSeriesList dtlEs = new DoubleTimeSeriesList();
		DoubleTimeSeries tsFundAvgEs = new DoubleTimeSeries();
		
		// No value investors? Set entries in tsFundAvgEs to zero and return
		if (valueTraders.size() == 0) {
		    tsFundAvgEs.fillWithConstants((int) nTicks, 0.0);
		    return tsFundAvgEs;
		}
		
		// Create a time series list of ES level of all value investors
        for (String key : valueTraders.keySet()) {
        	dtlEs.add(valueTraders.get(key).getTsEsPostTrade());
        }
        
        int nTicks = dtlEs.get(0).size();
        
        for (int i = 0; i < nTicks; i++) {  // calculate the aggregated ES at each tick by summing over all value investors
        	double totalEs = 0.0;
        	
        	for (int j = 0; j < dtlEs.size(); j++) {
        		totalEs += dtlEs.get(j).getValue(i);
        	}
        	
        	totalEs = totalEs / valueTraders.size();  // Divide by the number of value investors
        	
        	tsFundAvgEs.add(i, totalEs);
        }
    ...
    /**
     *  Calculate the ES time series averaged over trend followers  
     *    
     *  @return the time series of ES level
     */
	public DoubleTimeSeries getTrendAvgEs() { 
		
		HashMap<String, Trader> trendTraders = getTrendFollowers();
		DoubleTimeSeriesList dtlEs = new DoubleTimeSeriesList();
		DoubleTimeSeries tsTrendAvgEs = new DoubleTimeSeries();
		
		// No trend followers? Set entries in tsTrendAvgEs to zero and return
		if (trendTraders.size() == 0) {
		    tsTrendAvgEs.fillWithConstants((int) nTicks, 0.0);
		    return tsTrendAvgEs;
		}
		
		// Create a time series list of ES level of all trend followers
        for (String key : trendTraders.keySet()) {
        	dtlEs.add(trendTraders.get(key).getTsEsPostTrade());
        }
        
        int nTicks = dtlEs.get(0).size();
        
        for (int i = 0; i < nTicks; i++) {  // calculate the aggregated ES at each tick by summing over all trend followers
        	double totalEs = 0.0;
        	...
        	totalEs = totalEs / trendTraders.size();  // Divide by the number of trend followers
        	
        	tsTrendAvgEs.add(i, totalEs);
        }
    ...
    /**
     *  Calculate the stressed ES time series averaged over fundamental investors  
     *    
     *  @return the time series of stressed ES level
     */
	public DoubleTimeSeries getFundAvgStressedEs() { 
		
		HashMap<String, Trader> valueTraders = getValueInvestors();
		DoubleTimeSeriesList dtlStressedEs = new DoubleTimeSeriesList();
		DoubleTimeSeries tsFundAvgStressedEs = new DoubleTimeSeries();
		
		// No value investors? Set entries in tsFundAvgStressedEs to zero and return
		if (valueTraders.size() == 0) {
		    tsFundAvgStressedEs.fillWithConstants((int) nTicks, 0.0);
		    return tsFundAvgStressedEs;
		}
		
		// Create a time series list of stressed ES level of all value investors
        for (String key : valueTraders.keySet()) {
        	dtlStressedEs.add(valueTraders.get(key).getTsStressedEsPostTrade());
        }
        
        int nTicks = dtlStressedEs.get(0).size();
        
        for (int i = 0; i < nTicks; i++) {  // calculate the aggregated stressed ES at each tick by summing over all value investors
        	double totalStressedEs = 0.0;
        	
        	for (int j = 0; j < dtlStressedEs.size(); j++) {
        		totalStressedEs += dtlStressedEs.get(j).getValue(i);
        	}
        	
        	totalStressedEs = totalStressedEs / valueTraders.size();  // Divide by the number of value investors
        	
        	tsFundAvgStressedEs.add(i, totalStressedEs);
        }
    ...
    /**
     *  Calculate the stressed ES time series averaged over trend followers  
     *    
     *  @return the time series of stressed ES level
     */
	public DoubleTimeSeries getTrendAvgStressedEs() { 
		
		HashMap<String, Trader> trendTraders = getTrendFollowers();
		DoubleTimeSeriesList dtlStressedEs = new DoubleTimeSeriesList();
		DoubleTimeSeries tsTrendAvgStressedEs = new DoubleTimeSeries();
		
		// No trend followers? Set entries in tsTrendAvgStressedEs to zero and return
		if (trendTraders.size() == 0) {
		    tsTrendAvgStressedEs.fillWithConstants((int) nTicks, 0.0);
		    return tsTrendAvgStressedEs;
		}
		
		// Create a time series list of stressed ES level of all trend followers
        for (String key : trendTraders.keySet()) {
        	dtlStressedEs.add(trendTraders.get(key).getTsStressedEsPostTrade());
        }
        
        int nTicks = dtlStressedEs.get(0).size();
        
        for (int i = 0; i < nTicks; i++) {  // calculate the aggregated stressed ES at each tick by summing over all trend followers
        	double totalStressedEs = 0.0;
        	...
        	totalStressedEs = totalStressedEs / trendTraders.size();  // Divide by the number of trend followers
        	
        	tsTrendAvgStressedEs.add(i, totalStressedEs);
        }
