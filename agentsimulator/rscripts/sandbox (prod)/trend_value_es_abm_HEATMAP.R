...
###################################################################
#                                                                 #
#           Plot of heatmap of instability indicators             #
...
home.dir <- paste(root.dir, "/agentsimulator/out/trend-value-es-abm-simulation/Heatmap/", sep="")
...
#volatility_ret_avg_A1_es <- volatility_ret_avg_A1
#kurtosis_ret_avg_A1_es <- kurtosis_ret_avg_A1
#hill_ret_avg_A1_es <- hill_ret_avg_A1
#sharpe_avg_A1_es <- sharpe_avg_A1

#volatility_ret_avg_A1_var <- volatility_ret_avg_A1
#kurtosis_ret_avg_A1_var <- kurtosis_ret_avg_A1
#hill_ret_avg_A1_var <- hill_ret_avg_A1
#sharpe_avg_A1_var <- sharpe_avg_A1

volatility_ret_avg_A1_stressedes <- volatility_ret_avg_A1
kurtosis_ret_avg_A1_stressedes <- kurtosis_ret_avg_A1
hill_ret_avg_A1_stressedes <- hill_ret_avg_A1
sharpe_avg_A1_stressedes <- sharpe_avg_A1
...
wireframe(z, shade = FALSE, aspect = c(25/20, 20/25), xlab = "L�mite VaR", ylab = "Ventana volatilidad", zlab="", 
...
filled.contour3 <- function(
  ...
  axes = TRUE,
  frame.plot = axes,
  ...   # <- keep ... for xlab/ylab/cex passed from your call
) {

  # --- argument normalization, same logic as original ---
  if (missing(z)) {
    if (!missing(x)) {
      if (is.list(x)) {
        z <- x$z
  ...
  # --- set up 1-panel plotting region (no legend layout) ---
  plot.new()
  plot.window(
    xlim,
    ylim,
  ...
  # --- THIS is the key fix ---
  # Old: .Internal(filledcontour(...))  <-- removed in new R
  # New: use .filled.contour(...)
  .filled.contour(
    as.double(x),
    as.double(y),
    z,
...
png("heatmap_volatility.png", width = 3200, height = 900, res = 200)
layout(matrix(1:4, nrow = 1), widths = c(1, 1, 1, 0.12))

z_min <- min(volatility_ret_avg_A1_var, volatility_ret_avg_A1_es, volatility_ret_avg_A1_stressedes)
z_max <- max(volatility_ret_avg_A1_var, volatility_ret_avg_A1_es, volatility_ret_avg_A1_stressedes)
levels <- pretty(c(z_min, z_max), n = 20)
col    <- rev(heat.colors(length(levels) - 1))
ticks  <- pretty(c(z_min, z_max), n = 6)   # pocas marcas para el eje de leyenda

par(oma = c(0, 0, 0, 0))          # sin márgenes exteriores
par(las = 1, mgp = c(2.2, 0.6, 0))# números más cerca del eje, horizontales

## -------- Panel 1 --------
par(mar = c(6, 4, 4, 2))          # más margen inferior para eje X
z <- volatility_ret_avg_A1_var
filled.contour3(
  x, y, z,
  levels = levels, col = col,
  xlim = range(x), ylim = range(y), zlim = c(z_min, z_max),
  plot.axes = {
    axis(1, at = 1:nLVar,  labels = x_axis, cex.axis = 0.9, tcl = -0.25)
    axis(2, at = 1:nVolW, labels = y_axis, cex.axis = 0.9, tcl = -0.25)
  },
  plot.title = title(main = "VaR", xlab = "LimES", ylab = "Volatility window",
...
filled.contour3(
  ...
  plot.title = title(main = "ES", xlab = "LimES", ylab = "Volatility window",
                     cex.lab = 1.4, cex.main = 1.6)
)

## -------- Panel 3 --------
par(mar = c(6, 4, 4, 2))
z <- volatility_ret_avg_A1_stressedes
filled.contour3(
  x, y, z,
  levels = levels, col = col,
  xlim = range(x), ylim = range(y), zlim = c(z_min, z_max),
  ...
  plot.title = title(main = "Stressed ES", xlab = "LimES", ylab = "Volatility window",
                     cex.lab = 1.4, cex.main = 1.6)
)

## -------- shared legend (4th panel) --------
par(mar = c(1.5, 0.5, 1.5, 1.5))  # márgenes minúsculos para que quepa
plot.new()
filled.legend(
  x = x, y = y, z = z,
  levels = levels, col = col,
  xlim = c(0, 1), ylim = range(levels), zlim = c(z_min, z_max),
  axes = FALSE,
  key.axes = axis(4, at = ticks, labels = format(ticks, digits = 3),
                  cex.axis = 0.8, tcl = -0.25)  # pocas marcas y texto pequeño
)
...
png("heatmap_kurtosis.png", width = 3200, height = 900, res = 200)
layout(matrix(1:4, nrow = 1), widths = c(1, 1, 1, 0.12))

z_min <- min(kurtosis_ret_avg_A1_var, kurtosis_ret_avg_A1_es, kurtosis_ret_avg_A1_stressedes)
z_max <- max(kurtosis_ret_avg_A1_var, kurtosis_ret_avg_A1_es, kurtosis_ret_avg_A1_stressedes)
levels <- pretty(c(z_min, z_max), n = 20)
col    <- rev(heat.colors(length(levels) - 1))
ticks  <- pretty(c(z_min, z_max), n = 6)   # pocas marcas para el eje de leyenda
...
## -------- Panel 3 --------
par(mar = c(6, 4, 4, 2))
z <- kurtosis_ret_avg_A1_stressedes
filled.contour3(
  x, y, z,
  levels = levels, col = col,
  xlim = range(x), ylim = range(y), zlim = c(z_min, z_max),
...
png("heatmap_hill.png", width = 3200, height = 900, res = 200)
layout(matrix(1:4, nrow = 1), widths = c(1, 1, 1, 0.12))

z_min <- min(hill_ret_avg_A1_var, hill_ret_avg_A1_es, hill_ret_avg_A1_stressedes)
z_max <- max(hill_ret_avg_A1_var, hill_ret_avg_A1_es, hill_ret_avg_A1_stressedes)
levels <- pretty(c(z_min, z_max), n = 20)
col    <- heat.colors(length(levels) - 1)  # <- sin rev: bajo=rojo, alto=amarillo
ticks  <- pretty(c(z_min, z_max), n = 6)
...
z_min <- min(sharpe_avg_A1_var, sharpe_avg_A1_es, sharpe_avg_A1_stressedes)
z_max <- max(sharpe_avg_A1_var, sharpe_avg_A1_es, sharpe_avg_A1_stressedes)

png("heatmap_sharpe.png", width = 3200, height = 900, res = 200)
layout(matrix(1:4, nrow = 1), widths = c(1, 1, 1, 0.12))
...
filled.contour3(x, y, z, color=heat.colors, xlab = "",ylab = "", xlim = c(min(x),max(x)), ylim = c(min(y),max(y)), zlim = c(z_min,z_max),
      plot.axes={axis(1,at=1:nLVar, labels=x_axis, cex.axis=1.25)
		     axis(2,at=1:nVolW, labels=y_axis, cex.axis=1.25)},
      plot.title = title(main = "VaR",
		     xlab = "LimES", ylab = "Volatility window", cex.lab=1.5, cex.main=1.75) )

z <- sharpe_avg_A1_counter
filled.contour3(x, y, z, color=heat.colors, xlab = "",ylab = "", xlim = c(min(x),max(x)), ylim = c(min(y),max(y)), zlim = c(z_min,z_max),
      plot.axes={axis(1,at=1:nLVar, labels=x_axis, cex.axis=1.25)
		     axis(2,at=1:nVolW, labels=y_axis, cex.axis=1.25)},
      plot.title = title(main = "ES",
		     xlab = "LimES", ylab = "Volatility window", cex.lab=1.5, cex.main=1.75) )
...
filled.contour3(x, y, z, color=heat.colors, xlab = "",ylab = "", xlim = c(min(x),max(x)), ylim = c(min(y),max(y)), zlim = c(z_min,z_max),
      plot.axes={axis(1,at=1:nLVar, labels=x_axis, cex.axis=1.25)
		     axis(2,at=1:nVolW, labels=y_axis, cex.axis=1.25)},
      plot.title = title(main = "Stressed ES",
		     xlab = "LimES", ylab = "Volatility window", cex.lab=1.5, cex.main=1.75) )
